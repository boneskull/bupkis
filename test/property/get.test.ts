...
const defaultNumRuns = process.env.WALLABY ? 50 : process.env.CI ? 500 : 1000;
...
describe('get function property tests', () => {
  ...
  describe('failure modes and edge cases', () => {
    ...
    it('should handle primitive values as objects gracefully', () => {
      fc.assert(
        fc.property(
          fc.tuple(primitiveValue, fc.string({ minLength: 1 }), fc.anything()),
  ...
  describe('type safety and return values', () => {
    ...
    it('should work with deeply nested structures generated dynamically', () => {
      fc.assert(
        fc.property(
          fc.array(simpleKey, { maxLength: 3, minLength: 1 }).chain((keys) => {
